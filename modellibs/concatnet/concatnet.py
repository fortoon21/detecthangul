...
class ResNet(nn.Module):
    ...
    def forward(self, x):
        ...
        x_first_obo = self.layer3_attention_first(torch.cat((x_first, x_config), 1))
        x_middle_obo = self.layer3_attention_middle(torch.cat((x_middle, x_config), 1))
        x_last_obo = self.layer3_attention_last(torch.cat((x_last, x_config), 1))
